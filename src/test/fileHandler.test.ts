...
describe("FileHandler Test Suite", () => {
  ...
  describe("formatFilePath", () => {
    ...
    it("should handle paths with mixed special characters", () => {
      const uri = vscode.Uri.file(
        "/workspace/src/components/Button (legacy).tsx"
      );
      testContext.stubs.asRelativePath.returns(
        "src/components/Button (legacy).tsx"
      );
  ...
  describe("findCLITerminal", () => {
    ...
    it("should return first matching CLI terminal", () => {
      ...
      mockTerminals.set(
        "activePane",
        cliTerminal2 as unknown as vscode.Terminal
      );
      const terminals = [
        cliTerminal1,
        cliTerminal2,
  ...
  describe("sendFilesToTerminal", () => {
    it("should send single file to terminal", async () => {
      ...
      mockTerminals.set(
        "newPane",
        geminiTerminal as unknown as vscode.Terminal
      );
      testContext.sandbox
        .stub(vscode.window, "terminals")
        .value([geminiTerminal]);
      ...
      assert.ok(
        testContext.stubs.showInformationMessage.calledWith(
          "Sent 1 item to Gemini CLI"
        )
      );
    });
    ...
    it("should send multiple files to terminal", async () => {
      ...
      assert.ok(
        geminiTerminal.sendText.calledWith(
    ...
    it("should handle files with spaces in names", async () => {
      ...
      testContext.stubs.asRelativePath
        .onCall(1)
        .returns("test files/test.spec.ts");
      ...
      assert.ok(
        geminiTerminal.sendText.calledWith(
          '@"my docs/readme.md" @"test files/test.spec.ts" ',
          false
        )
      );
    });
    ...
    it("should show warning when no terminal exists", async () => {
      ...
      assert.ok(
        testContext.stubs.showWarningMessage.calledWith(
          "No AI CLI is running. Please start Gemini or Codex CLI first."
        )
      );
    });

    it("should show warning when no items provided", async () => {
      await fileHandler.sendFilesToTerminal([] as vscode.Uri[]);

      assert.ok(
        testContext.stubs.showWarningMessage.calledWith("No item selected")
      );
    });
