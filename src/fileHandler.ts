...
export class FileHandler {
    ...
    constructor(geminiTerminals: Map<string, vscode.Terminal>) {
    ...
    findGeminiTerminal(terminals: readonly vscode.Terminal[]): vscode.Terminal | undefined {
    ...
    async sendFilesToTerminal(uris: vscode.Uri[] | vscode.Uri): Promise<void> {
        // Handle single URI or array
        const uriArray = Array.isArray(uris) ? uris : [uris];
        
        // Debug log
        console.log('sendFilesToTerminal called with:', uriArray.map(u => u.fsPath));
        ...
        const terminal = this.findGeminiTerminal(vscode.window.terminals);
        if (!terminal) {
        ...
        vscode.window.showInformationMessage(
            `Sent ${itemCount} item${itemCount > 1 ? 's' : ''} to Gemini CLI`
        );
    }
}