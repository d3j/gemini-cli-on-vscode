...
export class FileHandler {
  ...
  findCLITerminal(
    ...
    targetCLI?: "gemini" | "codex" | "claude" | "qwen"
    ...
    // Priority: Claude -> Codex -> Gemini -> Qwen
    for (const cli of ["claude", "codex", "gemini", "qwen"] as const) {
  ...
  async sendFilesToTerminal(
    ...
    // Debug log
    console.log(
      "sendFilesToTerminal called with:",
      uriArray.map((u) => u.fsPath),
      "target:",
      targetCLI
    );
    ...
    if (!terminal) {
      ...
      const message = targetCLI
        ...
        : "No CLI is running. Please start Gemini, Codex, Claude or Qwen CLI first.";
